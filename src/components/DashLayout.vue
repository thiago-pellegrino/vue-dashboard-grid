<template>
  <div v-if="currentBreakpoint === breakpoint">
    <div v-if="debug">
      ...
      Layout Number of Rows: {{ numberOfRows }} <br />
      placeholder: {{ JSON.stringify(placeholder) }} <br />
      Items: {{ JSON.stringify(itemsFromLayout) }} <br />
      Height: {{ height }} <br />
      placeholderMaxWidth: {{ placeholderMaxWidth }} <br />
      Attrs: {{ $attrs }}
    </div>    
    <div v-if="l" :style="{ position: 'relative', height: height, width: width }">
      <slot></slot>
      <DashItem :id="placeholderId" :draggable="false" :resizable="false" v-show="dragging || resizing" :y.sync="placeholderY" :height.sync="placeholderHeight" :maxWidth.sync="placeholderMaxWidth">
